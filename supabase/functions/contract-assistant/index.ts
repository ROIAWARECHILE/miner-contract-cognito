...
serve(async (req) => {
  ...
  try {
    ...
    if (!contextChunks || contextChunks.length === 0) {
      // If no context is found, return a helpful message without calling OpenAI
      const readableStream = new ReadableStream({
        start(controller) {
          const message = "Aún no he podido analizar los documentos de este contrato. Una vez que los documentos hayan sido procesados, podré responder a tus preguntas.";
