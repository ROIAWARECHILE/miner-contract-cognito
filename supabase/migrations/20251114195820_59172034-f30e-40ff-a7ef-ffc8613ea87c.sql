-- Fix delete_contract_cascade function - entity_id type mismatch
...
BEGIN
  ...
  -- Log deletion in audit_log (FIX: entity_id is UUID, not text)
  INSERT INTO audit_log (entity_type, entity_id, action, user_id, user_email)
  VALUES ('contract', p_contract_id, 'delete', auth.uid(), (SELECT email FROM auth.users WHERE id = auth.uid()));
